...
### 7.6 `renderer.ts` 和 `scheduler.ts` 里两处 `flushPostFlushCbs()` 有什么区别？

这两个调用刷的是同一个 post 队列，但职责不一样。

先看共同点：

- 它们最终处理的都是 `pendingPostFlushCbs`
- 队列里的内容通常来自 `queuePostRenderEffect(...)`
- 非 Suspense 场景下，`queuePostRenderEffect` 最终就是 `queuePostFlushCb`

真正要分清的是“谁在什么阶段触发这次 flush”。

#### `scheduler.ts` 里的调用：调度器一轮 flush 的标准收尾

在 `packages/runtime-core/src/scheduler.ts` 的 `flushJobs()` 里，Vue 会先跑完整个主 job 队列，最后再执行：
...
queueJob(...)
  -> queueFlush()
  -> flushJobs()
  -> 先跑主队列里的 component update / pre jobs
  -> 再跑 flushPostFlushCbs(seen)
```

所以这里的含义是：

- 主更新任务已经跑完
- 现在统一执行这一轮积累下来的 post 回调
- 如果 post 回调里又塞进了新的 job 或新的 post cb，`flushJobs()` 还会继续往下刷

也就是说，`scheduler.ts` 这处调用对应的是“整个调度周期的尾声”。

#### `renderer.ts` 里的调用：根 render 结束后的兜底 flush

在 `packages/runtime-core/src/renderer.ts` 的根 `render()` 末尾，Vue 还会手动调用一次：
...
这里不是在跑 scheduler 主队列，而是在“这次同步 render / patch 已经结束”之后，顺手把这一轮 render 过程中产生的 pre / post 回调清掉。

它更像一个兜底动作：

- 有些回调是在同步 `render()` 过程中直接通过 `queuePostRenderEffect(...)` 塞进去的
- 但当前这次流程不一定是从 `queueJob -> flushJobs` 这条异步调度链路进来的
- 所以根 `render()` 自己要在结尾补一次 `flushPostFlushCbs()`
...
也就是说，`renderer.ts` 这处调用对应的是“根渲染入口在同步 patch 结束后的主动清场”。

#### 一句话区分

- `scheduler.ts`：调度器一轮 `flushJobs()` 跑完主队列后的标准收尾
- `renderer.ts`：根 `render()` 同步执行完后的兜底 flush

所以不要把它们看成“两套 post 队列”，它们刷的是同一批回调；区别只在于：

- 一个属于 scheduler 的调度周期尾部
- 一个属于 renderer 的渲染入口尾部
...
而 `flushPostFlushCbs()` 到底是被 `renderer.ts` 调起，还是被 `scheduler.ts` 调起，要看这批回调是落在“同步根 render”里，还是“异步 job flush”里。
